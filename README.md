...
the `flake.nix` is 3 lines: `{ outputs = _: (import ./assemble.nix) {}; }`. it declares no inputs of its own (tack owns those) so theres no `flake.lock`. it exists ONLY so the `.#` CLI works.

{
  ...
  in {
    config = {
      users.users.${config.mySystem.user.name}.packages = [pkgs.oh-my-posh];

      hjem.users.${config.mySystem.user.name} = {
        xdg.config.files."oh-my-posh/config.json".text = ompConfig;

        xdg.config.files."fish/conf.d/oh-my-posh.fish".text = ''

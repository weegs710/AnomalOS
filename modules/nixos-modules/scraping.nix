{
  flake.nixosModules.scraping = {
    ...
    yoinkScript = ''
      ...
      # Download YouTube videos/playlists in 1080p H.264/AAC MP4 format.
      # Output goes to ~/Videos/yt-dlp/

      def main [url: string] {
          let video_dir = $env.HOME | path join "Videos" "yt-dlp"
          mkdir $video_dir

          print $"(ansi blue)Downloading...(ansi reset)"
          print $"(ansi yellow)Format: 1080p H.264 + AAC audio \(MP4\)(ansi reset)"

          let args = [
              "-f" "299+140-12/bestvideo[height<=1080][vcodec^=avc]+bestaudio[acodec^=mp4a]/best[height<=1080]"
              "--merge-output-format" "mp4"
              "--ignore-errors"
              "-o" ($video_dir | path join "%(title)s.%(ext)s")
              $url
          ]
          ...
          if $exit_code == 0 {
              print $"(ansi green)Done! Saved to: ($video_dir)(ansi reset)"
          } else {
              print $"(ansi red)yt-dlp exited with code ($exit_code)(ansi reset)"
              exit $exit_code
          }
      }
    '';

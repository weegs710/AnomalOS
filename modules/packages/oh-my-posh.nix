# Wrapped oh-my-posh with custom theme
# Run with: nix run github:weegs710/AnomalOS#oh-my-posh
{ ... }: {
  perSystem = { pkgs, ... }:
    let
      colors = {
      ...
      configFile = pkgs.writeText "oh-my-posh-config.json" (builtins.toJSON {
      ...
      wrappedOhMyPosh = pkgs.symlinkJoin {
        name = "oh-my-posh-wrapped";
        paths = [ pkgs.oh-my-posh ];
        nativeBuildInputs = [ pkgs.makeWrapper ];
        postBuild = ''
          wrapProgram $out/bin/oh-my-posh \
    ...
    in
    {
      packages.oh-my-posh = wrappedOhMyPosh;
    };
}
